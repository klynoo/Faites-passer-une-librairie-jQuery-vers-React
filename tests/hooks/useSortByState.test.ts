...
import useSortByState from "../../src/hooks/useSortByState"

describe("useSortByState", () => {
  it("tri par défaut en ordre alphabétique ascendant (state)", () => {
    const { result } = renderHook(() => useSortByState())
    const { sortedEmployees } = result.current

    expect(sortedEmployees.map((emp) => emp.state)).toEqual(["DE", "FR", "UK"])
  })
  ...
  it("passe en ordre descendant après toggleSortOrder", () => {
    ...
    const { sortedEmployees } = result.current
    expect(sortedEmployees.map((emp) => emp.state)).toEqual(["UK", "FR", "DE"])
  })
})
