...
import useSortByFirstName from "../../src/hooks/useSortByFirstName"

describe("useSortByFirstName", () => {
  it("tri par défaut en ordre ascendant (firstName)", () => {
    const { result } = renderHook(() => useSortByFirstName())
    const { sortedEmployees } = result.current

    expect(sortedEmployees.map((emp) => emp.firstName)).toEqual([
      "Alice",
      "Jane",
      "John",
    ])
