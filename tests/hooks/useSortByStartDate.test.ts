...
import useSortByStartDate from "../../src/hooks/useSortByStartDate"

describe("useSortByStartDate", () => {
  it("tri par défaut en ordre chronologique (ascendant)", () => {
    const { result } = renderHook(() => useSortByStartDate())
    const { sortedEmployees } = result.current

    expect(sortedEmployees.map((emp) => emp.startDate)).toEqual([
      "2019-03-10", // Alice
      "2020-01-01", // John
      "2021-06-15", // Jane
    ])
